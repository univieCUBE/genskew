...
def save():
    ...
    try:
        ...
        for sequence in sequence_list:
            ...
            gs.plot_sequence(results_list[a], filenames[a], output_folder, out_filetype, dpi, skewi)
            a = a + 1
    except:
    ...
    if len(filenames) == 1:
        ...
        if 1 == 1:
            ...
            if alg == "SkewIT":
                ...
                # Print to file
                axis.plot(results.x, pos, 'deepskyblue', results.x, mid, 'g', results.x, neg, 'k')
                axis.set(xlabel='Genome Position', ylabel= results.nuc_1 + " " + results.nuc_2 + " skew")
                ...
                axis.xaxis.set_major_formatter(tick.FuncFormatter(reformat_bp))
                axis.grid()
                axis.legend()
                axis.set_title("SkewIT plot for sequence: " + filenames[0].split('/')[-1] + ", with stepsize: " + str(
